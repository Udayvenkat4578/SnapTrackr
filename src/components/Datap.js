...
const [val,setVal]=useState([]);
 ...
 useEffect(() => {
    axios.get(`https://api.unsplash.com/search/photos?page=10&per_page=100&client_id=JJEqtqwDIAe49O0Gk-6uQXePh88gNJW8SuMisZWsvG0&query=animation`).then((res)=>{setData(res.data.results)})
...
const image =()=>{
  axios.get(`https://api.unsplash.com/search/photos?page=10&per_page=100&client_id=JJEqtqwDIAe49O0Gk-6uQXePh88gNJW8SuMisZWsvG0&query=${val}`).then((res)=>{setData(res.data.results)})
}

  return (
    <div>
      <Nav1/>
                    <div className='opacity-80 hidden  lg:flex '>
                <ul className='flex flex-row pt-3  '>
                <li className='md:mr-5 mr-2 font-medium md:text-lg text-xs  border border-md border-gray-300 rounded-sm px-3 py-1 text-gray-700 ml-5'><button onClick={()=>{axios.get("https://api.unsplash.com/search/photos?page=10&per_page=100&client_id=JJEqtqwDIAe49O0Gk-6uQXePh88gNJW8SuMisZWsvG0&query=wallpaper").then((res)=>{setData(res.data.results)})}}>Wallpaper</button> </li> 
                <li className='md:mr-5 mr-2 font-medium md:text-lg text-xs  border border-md border-gray-300 rounded-sm px-3 py-1 text-gray-700 '><button onClick={()=>{axios.get("https://api.unsplash.com/search/photos?page=10&per_page=100&client_id=JJEqtqwDIAe49O0Gk-6uQXePh88gNJW8SuMisZWsvG0&query=Aeroplane").then((res)=>{setData(res.data.results)})}}>Aeroplane</button>  </li> 
